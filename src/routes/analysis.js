...
// Add this route to analysisRoutes.js (before module.exports)

// Batch soil analysis endpoint
router.post('/analyze-soil-batch', async (req, res) => {
    try {
        const { 
            userId, 
            soilSamples,  // Array of soil samples (max 10)
            batchId,
            cropType = 'tomato',
            storeResults = false  // Whether to store in database
        } = req.body;
        
        console.log(`🌱 [BACKEND] Starting batch soil analysis for user ${userId}`);
        console.log(`📊 Number of soil samples: ${soilSamples?.length || 0}`);
        ...
        if (!soilSamples || !Array.isArray(soilSamples) || soilSamples.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'Soil samples array is required',
                received: soilSamples
            });
        }
        
        if (soilSamples.length > 10) {
            return res.status(400).json({
                success: false,
                message: 'Maximum 10 soil samples per batch',
                maxAllowed: 10,
                received: soilSamples.length
            });
        }
        
        // Validate each soil sample has required fields
        const requiredFields = ['ph_level', 'moisture', 'temperature', 'nitrogen', 'phosphorus', 'potassium'];
        const validationErrors = [];
        
        soilSamples.forEach((sample, idx) => {
        ...
        if (validationErrors.length > 0) {
            return res.status(400).json({
                success: false,
                message: 'Invalid soil samples data',
        ...
        const batchResult = await mlService.analyzeSoilBatch(
            soilSamples,
            userId,
            batchId,
            cropType
        ...
        if (storeResults && storageService.storeBatchSoilResults) {
            storedResults = await storageService.storeBatchSoilResults(
                batchResult.results,
                userId,
                batchResult.batch_id
            );
            console.log(`💾 Stored ${storedResults?.stored_count || 0} soil analysis results`);
        }
        ...
        const response = {
            ...
            total_samples: batchResult.total_samples,
            successful_analyses: batchResult.successful_analyses,
            failed_analyses: batchResult.failed_analyses,
            ...
            average_time_per_sample_ms: batchResult.average_time_per_sample_ms,
            stored: !!storedResults,
            storage_details: storedResults,
            user_id: userId,
            crop_type: cropType,
        ...
        if (batchResult.summary) {
            response.summary = batchResult.summary;
        }
        
        console.log(`✅ Batch soil analysis completed successfully`);
        console.log(`📊 Summary: ${batchResult.successful_analyses}/${batchResult.total_samples} successful`);
        
        res.json(response);
        
    } catch (error) {
        console.error('❌ Batch soil analysis failed:', error);
        res.status(500).json({
            success: false,
            message: 'Batch soil analysis failed: ' + error.message,

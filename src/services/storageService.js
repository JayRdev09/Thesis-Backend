...
formatSoilIssuesForStorage(soilIssues) {
  ...
  // Add this method to your storageService.js file
...
async deleteAnalysis(predictionId, userId) {
  try {
    ...
    // First, get the analysis to check if it exists and belongs to user
    const { data: analysis, error: fetchError } = await this.client
      .from('predictions')
      .select('prediction_id, user_id, image_id')
      .eq('prediction_id', predictionId)
      .eq('user_id', userId)
      .single();
    
    if (fetchError || !analysis) {
      console.log(`❌ Analysis ${predictionId} not found or doesn't belong to user ${userId}`);
      return false;
    }
    ...
    console.log(`✅ Successfully deleted analysis ${predictionId}`);
    return true;
...
async deleteImage(imageId, userId) {
  try {
    ...
    // First, delete any analysis predictions linked to this image
...
// Delete all images for a user (optional)
async deleteAllImages(userId) {
  try {
    console.log(`🗑️ Deleting all images for user ${userId}`);
    
    // Get all images for the user
    const { data: images, error: fetchError } = await this.client
      .from('images')
    ...
    const { error: predictionsError } = await this.client
      .from('predictions')
      .delete()
      .in('image_id', images.map(img => img.image_id))
      .eq('user_id', userId);
    ...
    const storagePaths = images.filter(img => img.image_path).map(img => img.image_path);
    ...
    console.log(`✅ Successfully deleted ${images.length} images for user ${userId}`);
